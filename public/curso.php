...
mysqli_stmt_bind_param($stmt, "ii", $_SESSION["usuario_id"], $curso_id);
mysqli_stmt_execute($stmt);
...
<body>
    ...
    <?php while ($leccion = mysqli_fetch_assoc($resultado_lecciones)):
        // Comprobar si esta lección está completada
        $sql_check = "SELECT id FROM progreso 
              WHERE usuario_id = ? AND leccion_id = ?";

        $stmt_check = mysqli_prepare($conexion, $sql_check);
        mysqli_stmt_bind_param($stmt_check, "ii", $_SESSION["usuario_id"], $leccion["id"]);
